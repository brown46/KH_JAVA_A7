...
public class MyinfoController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		updateData.setPassword(newPassowrd);
		updateData.setEmail(email);
		
		UserService service = new UserService();
		UserDTO result= service.update(data,updateData,oldPassword);
		if(result == null) {
			req.setAttribute("error", "패스워드가 일치하지 않거나 저장에 문제가 발생하였습니다");
			req.getRequestDispatcher("/WEB-INF/view/myinfo.jsp");
		}else {
