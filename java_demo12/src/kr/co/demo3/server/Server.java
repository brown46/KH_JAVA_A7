...
public class Server {
	...
	public void run() {
		try {
			...
			clientOut = new BufferedOutputStream(sock.getOutputStream());
			...
			while(true) {
				while(true) {
					...
					if(new String(buf,0,9).equals("fileName=")) {
						fileName= new String(buf,9,len);
					}else {
						this.save(fileName,buf, len);
					}
				}
			}
		} catch (IOException e) {
	...
	public void save(String fileName,byte[] byteData,int length) {
