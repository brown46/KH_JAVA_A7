...
public class AccountController {
	...
	public AccountVO login(String nickname, String password) {
		...
		if(loginData !=null) {
			...
			view.message("로그인 정보가 잘못 되었습니다. - 로그인 실퍂  	");
		}
	...
	public void setPassword(AccountVO user,String nPassword) {
		AccountDAO dao = new AccountDAO();
		AccountView  view = new AccountView();
		 
		boolean result = dao.updateNewPassword(user,nPassword);
		if(result) {
			view.message("패스워드 변경 처리가 완료되었습니다. - 패스워드 변경 성공");
		}else {
			view.message("패스워드 처리 중 문제가 발생하였습니다. - 패스워드 변경 실패");
		}
