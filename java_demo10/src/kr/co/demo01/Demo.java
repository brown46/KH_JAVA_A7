...
public class Demo {
	...
	public static void main(String[] args) {
		 ...
		 * 	  내부/외부 장치로 나가면 출력 이라고 한다.
		 * 	  (하드디스크의 파일을 프로그램 내에서 사용할 수 있도록 불러올 때는 입력
		 * 	  반대로 프로그램에서 생성한 데이터를 하드디스크의 파일로 내보낼 때는 출력)
		 * 	- 입출력은 입력스트림, 출력스트림이 개별로 존재하며 입출력은 동시에 작업을 할 수 
		 * 	  없기 때문에 반드시 입력과 출력을 구분하여 진행해야 한다.
		 * 
		 * 스트림(Stream)
		 *  - 두 장치간에 데이터를 교환하기 위해 생성한 데이터 이동 통로
		 *  - 스트림은 입력/출력 스트림으로 나누어져 있고 또 그 안에서 바이트기반 스트림,
		 *    문자기반 스트림으로 분리되어 있다.
		 *  - 바이트기반 스트림은 입/출력에 사용하는 데이터가 바이트로 이루어짐.
		 *    (이미지, 영상 등의 바이트기반 데이터를 읽고 입출력 할 때 사용)
		 *  - 문자기반 스트림은 입/출력에 사용하는 데이터가 문자로 이루어짐.
		 *    (텍스트 문서등의 문자기반 데이터를 입출력 할 때 사용)
		 */

		File f = new File("D:\\입출력테스트.txt");
		System.out.printf("%d byte\n",f.length());
		
		System.out.printf("%1$tY년 %1$tm월 %1$td일",new Date(f.lastModified()));//수정한 날짜 확인
		...
		if(!f2.exists()) {
			f2.mkdir(); //파일 경로 생성
			//f2.mkdirs(); //상위 폴더가 없을 때 ex)"D:\\temp\\src\\project"
		}
