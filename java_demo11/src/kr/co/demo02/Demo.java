...
public class Demo {
	...
	public static void main(String[] args) {
		 ...
		 * 					동등 객체:	객체의 참조주소는 다르지만 멤퍼필드가 동일
		 * 
		 * LinkedHashSet
		 * 		- HashSet 과 동일하지만 Set에 추가되는 데이터들의 순서가 유지된다.
		 * 
		 * TreeSet
		...
		System.out.println("add "+aSet); //순서가 유지되지 않음
		...
		System.out.println("remove 20 "+ aSet);
		...
		System.out.println("List로 변경 "+aList);
		
		// List 를 Set으로 변경
		HashSet<Integer> bSet = new HashSet<Integer>(aList);
		System.out.println("Set으로 변경 "+bSet);
