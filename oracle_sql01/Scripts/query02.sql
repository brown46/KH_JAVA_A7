 ...
 * 		 조회 결과로 Rcord Set을 반환받게 된다.
 * 		 Record Set 에는 Record가 0개 이상 포함되어 있다.
 */
 ...
 *  FROM 절에는 조회할 테이블며을 작성하게 된다. 
 *  WHERE 절에는 FROM절에 명시한 테이블에서 데이터를 조회하기 위한 조건을 작성하게 된다.
 *  ORDER BY 절에는 조회 결과에 대한 정렬 기준을 작성하게 된다.
 *  SELECT 절에는 FROM 절에 명시한 테이블의 컬럼을 작성하게 딘다. 이때 작성된 컬럼의 
...
/*
 * SELECT 절에 사용하는 컬럼명이 Record Set 으로 반환 될 때 그대로 출력되지만
	이를 변경하고 싶으면 별칭을 부여하여 Record Set 으로 출력하는 컬럼명을 변경할 
	수 있다.
*/
...
/*
 * SELECT 절에 컬럼명과 리터럴 값을 혼합하여 Record Set으로 반환되는
 * 결과값을 변경 할 수 있다.
 */
...
/*
 * IS NULL, IS NOT NULL : NULL데이터를 찾기 위한 연산자로 사용
 */
SELECT FIRST_NAME
	, LAST_NAME
	, DEPARTMENT_ID
