...
SELECT JOB_ID
	 ...
	 , MIN(SALARY*(NVL(COMMISSION_PCT,0)+1)) AS 최저급여액
  FROM EMPLOYEES
 GROUP BY JOB_ID  ;
 ...
 * 			그룹별 집계 외에 단일 그룹별 집계 /복수 개의 그룹별 집계/ 총 집계
 * 			까지 나타낸다.
 */
SELECT JOB_ID 
	 , COMMISSION_PCT 
...
/*
 * GROUPING : ROLLUP, CUBE 로 집계한 결과에 대해 어떠한 조합으로 그룹을 묶었는지
 * 			  알 수 있도록 도와주는 함수(SELECT 절에 사용)
 * 			  함수의 반환값이 0이면 그룹에 포함된 것이며, 1이면 포함이 안된 것이다.
 */
SELECT JOB_ID 
	 , COMMISSION_PCT 
	 , COUNT(*) AS 수
...
SELECT JOB_ID 
	 ...
	 , COUNT(*) AS 수
	 , CASE WHEN GROUPING(JOB_ID)=0 AND GROUPING(COMMISSION_PCT)=1 THEN '직무그룹'
	 		WHEN  GROUPING(JOB_ID)=1 AND GROUPING(COMMISSION_PCT)=0  THEN '직무/커미션그룹'
	 		WHEN  GROUPING(JOB_ID)=0 AND GROUPING(COMMISSION_PCT)=0  THEN '직무/커미션그룹'
	 		ELSE '총계'
	 	END AS 그룹구분
  FROM EMPLOYEES 
