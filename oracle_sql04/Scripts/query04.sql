...
SELECT 순위
	 , (SELECT DEPARTMENT_NAME  
	      FROM DEPARTMENTS D
	     WHERE D.DEPARTMENT_ID =DEPT_ID) AS DEPT_NAME
	 , DEPT_ID
	 , 평균급여
  FROM (SELECT RANK() OVER(ORDER BY AVG(SALARY) DESC) AS 순위
...
SELECT 부서이름
	 ...
	 , 이름
	 , ROWNUM
  FROM (SELECT (SELECT DEPARTMENT_NAME 
                  FROM DEPARTMENTS DEPT
                 WHERE DEPT.DEPARTMENT_ID =E.DEPARTMENT_ID) AS 부서이름
	         , HIRE_DATE
